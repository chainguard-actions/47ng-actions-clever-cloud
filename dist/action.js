...
function processArguments() {
    ...
    return {
        ...
        appID,
        timeout,
        cleverCLI: node_path_1.default.resolve(__dirname, '../node_modules/.bin/clever'),
...
async function run({ token, secret, appID, alias, force = false, cleverCLI, timeout, logFile, quiet = false, extraEnv = {} }) {
    try {
        ...
        if (appID) {
            ...
            await (0, exec_1.exec)(cleverCLI, ['link', appID, '--alias', appID], execOptions);
            alias = appID;
        }
        ...
        for (const envName of Object.keys(extraEnv)) {
            ...
            await (0, exec_1.exec)(cleverCLI, args, execOptions);
        }
        const args = ['deploy'];
        if (appID) {
        ...
        if (timeout) {
            ...
            const timeoutPromise = new Promise(resolve => {
                timeoutID = (0, node_timers_1.setTimeout)(() => {
                    timedOut = true;
                    resolve();
                }, timeout);
            });
            const result = await Promise.race([
                (0, exec_1.exec)(cleverCLI, args, execOptions),
                timeoutPromise
            ]);
            if (timeoutID) {
                (0, node_timers_1.clearTimeout)(timeoutID);
            }
            if (timedOut) {
            ...
            const code = await (0, exec_1.exec)(cleverCLI, args, execOptions);
            core.info(`code: ${code}`);
            if (code !== 0) {
...
async function getOutputStream(quiet, logFile) {
    ...
    if (!quiet) {
        ...
        tee
            .pipe(node_stream_1.Transform.from(splitNewlines))
            .pipe(node_stream_1.Transform.from(injectAnnotations))
            .pipe(process.stdout);
    }
    if (logFile) {
        const logFileStream = (await promises_1.default.open(logFile, 'w')).createWriteStream();
