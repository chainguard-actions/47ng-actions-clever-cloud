...
const DEPLOYMENT_TIMEOUT_MESSAGE = "Deployment timed out, moving on with workflow run";
async function deploy(config, deps) {
	...
	if (await clever.deploy({
		...
		if (config.quiet && config.logFile) deployLog?.write(`${DEPLOYMENT_TIMEOUT_MESSAGE}\n`);
